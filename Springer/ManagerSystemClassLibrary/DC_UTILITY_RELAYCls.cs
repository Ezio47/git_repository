...
namespace ManagerSystemClassLibrary
{
    ...
    /// 数据中心_设施_中继站
    /// </summary>
    public class DC_UTILITY_RELAYCls
    {
        #region 增、删、改
        /// <summary>
        /// 增、删、改
        ...
        public static Message Manager(DC_UTILITY_RELAY_Model m)
        {
            ...
            {
                //SystemCls.LogSave("4", "通知公告:" + m.INFOTITLE, ClsStr.getModelContent(m));
                Message msgUser = BaseDT.DC_UTILITY_RELAY.MdyJWD(m);
                return new Message(msgUser.Success, msgUser.Msg, msgUser.Url);
            ...
            if (m.opMethod == "Del")
            {
                ...
                Message msgUser = BaseDT.DC_UTILITY_RELAY.Del(m);
                return new Message(msgUser.Success, msgUser.Msg, msgUser.Url);
            }
        ...
        /// <returns>参见模型</returns>
        public static DC_UTILITY_RELAY_Model getModel(DC_UTILITY_RELAY_SW sw)
        {
            var result = new List<DC_UTILITY_RELAY_Model>();

            DataTable dt = BaseDT.DC_UTILITY_RELAY.getDT(sw);//列表
        ...
        /// <param name="sw"></param>
        /// <returns></returns>
        public static IEnumerable<DC_UTILITY_RELAY_Model> getModelList(DC_UTILITY_RELAY_SW sw)
        {
            var result = new List<DC_UTILITY_RELAY_Model>();
        ...
        {
            var Count = "";
            Count = BaseDT.DC_UTILITY_RELAY.getNum(new DC_UTILITY_RELAY_SW { BYORGNO = SystemCls.getCurUserOrgNo() });
            return Count;
        }
        #endregion
    }
