...
namespace ManagerSystemClassLibrary
{
    ...
    /// 数据中心_设施_因子采集站
    /// </summary>
    public class DC_UTILITY_FACTORCOLLECTSTATIONCls
    {
        #region 增、删、改
        /// <summary>
        /// 增、删、改
        ...
        public static Message Manager(DC_UTILITY_FACTORCOLLECTSTATION_Model m)
        {
            if (m.opMethod == "Add")
            {
                //SystemCls.LogSave("3", "通知公告:" + m.INFOTITLE, ClsStr.getModelContent(m));
                Message msgUser = BaseDT.DC_UTILITY_FACTORCOLLECTSTATION.Add(m);
                return new Message(msgUser.Success, msgUser.Msg, msgUser.Url);
            }
            if (m.opMethod == "Mdy")
            {
                //SystemCls.LogSave("4", "通知公告:" + m.INFOTITLE, ClsStr.getModelContent(m));
                Message msgUser = BaseDT.DC_UTILITY_FACTORCOLLECTSTATION.Mdy(m);
                return new Message(msgUser.Success, msgUser.Msg, msgUser.Url);
            ...
            {
                //SystemCls.LogSave("4", "通知公告:" + m.INFOTITLE, ClsStr.getModelContent(m));
                Message msgUser = BaseDT.DC_UTILITY_FACTORCOLLECTSTATION.MdyJWD(m);
                return new Message(msgUser.Success, msgUser.Msg, msgUser.Url);
            ...
            if (m.opMethod == "Del")
            {
                ...
                Message msgUser = BaseDT.DC_UTILITY_FACTORCOLLECTSTATION.Del(m);
                return new Message(msgUser.Success, msgUser.Msg, msgUser.Url);
            }
        ...
        /// <returns>参见模型</returns>
        public static DC_UTILITY_FACTORCOLLECTSTATION_Model getModel(DC_UTILITY_FACTORCOLLECTSTATION_SW sw)
        {
            var result = new List<DC_UTILITY_FACTORCOLLECTSTATION_Model>();

            DataTable dt = BaseDT.DC_UTILITY_FACTORCOLLECTSTATION.getDT(sw);//列表
            ...
            {
                int i = 0;
                m.DC_UTILITY_FACTORCOLLECTSTATION_ID = dt.Rows[i]["DC_UTILITY_FACTORCOLLECTSTATION_ID"].ToString();
        ...
        /// <param name="sw"></param>
        /// <returns></returns>
        public static IEnumerable<DC_UTILITY_FACTORCOLLECTSTATION_Model> getModelList(DC_UTILITY_FACTORCOLLECTSTATION_SW sw)
        {
            var result = new List<DC_UTILITY_FACTORCOLLECTSTATION_Model>();
        ...
        /// <returns>参见模型</returns>
        public static IEnumerable<DC_UTILITY_FACTORCOLLECTSTATION_Model> getModelList(DC_UTILITY_FACTORCOLLECTSTATION_SW sw, out int total)
        {
            var result = new List<DC_UTILITY_FACTORCOLLECTSTATION_Model>();

            DataTable dt = BaseDT.DC_UTILITY_FACTORCOLLECTSTATION.getDT(sw, out total);//列表
        ...
        public static void FACTORCOLLECTSTATIONUpload(string filePath)
        {
            ...
            for (int i = (sheet.FirstRowNum + 1); i <= rowCount; i++)
            {
                ...
                var ms = BaseDT.DC_UTILITY_FACTORCOLLECTSTATION.Add(m);
                if (string.IsNullOrEmpty(jd) == false && string.IsNullOrEmpty(wd) == false)
                {
                    ...
                    BaseDT.SDE.TD_FACTORCOLLECTSTATION.Add(m1);
                }
        ...
        {
            var Count = "";
            Count = BaseDT.DC_UTILITY_FACTORCOLLECTSTATION.getNum(new DC_UTILITY_FACTORCOLLECTSTATION_SW { BYORGNO = SystemCls.getCurUserOrgNo() });
            return Count;
        }
        #endregion
    }
