...
namespace ManagerSystemClassLibrary
{
    ...
    /// 数据中心_设施_营房
    /// </summary>
    public class DC_UTILITY_CAMPCls
    {
        #region 增、删、改
        /// <summary>
        /// 增、删、改
        ...
        public static Message Manager(DC_UTILITY_CAMP_Model m)
        {
            ...
            {
                //SystemCls.LogSave("4", "通知公告:" + m.INFOTITLE, ClsStr.getModelContent(m));
                Message msgUser = BaseDT.DC_UTILITY_CAMP.MdyJWD(m);
                return new Message(msgUser.Success, msgUser.Msg, msgUser.Url);
            ...
            if (m.opMethod == "Del")
            {
                ...
                Message msgUser = BaseDT.DC_UTILITY_CAMP.Del(m);
                return new Message(msgUser.Success, msgUser.Msg, msgUser.Url);
            }
        ...
        /// <returns>参见模型</returns>
        public static DC_UTILITY_CAMP_Model getModel(DC_UTILITY_CAMP_SW sw)
        {
            var result = new List<DC_UTILITY_CAMP_Model>();

            DataTable dt = BaseDT.DC_UTILITY_CAMP.getDT(sw);//列表
            ...
            {
                int i = 0;
                m.DC_UTILITY_CAMP_ID = dt.Rows[i]["DC_UTILITY_CAMP_ID"].ToString();
        ...
        /// <param name="sw"></param>
        /// <returns></returns>
        public static IEnumerable<DC_UTILITY_CAMP_Model> getModelList(DC_UTILITY_CAMP_SW sw)
        {
            var result = new List<DC_UTILITY_CAMP_Model>();
        ...
        /// <returns>参见模型</returns>
        public static IEnumerable<DC_UTILITY_CAMP_Model> getModelList(DC_UTILITY_CAMP_SW sw, out int total)
        {
            var result = new List<DC_UTILITY_CAMP_Model>();

            DataTable dt = BaseDT.DC_UTILITY_CAMP.getDT(sw, out total);//列表
        ...
        public static void CAMPUpload(string filePath)
        {
            ...
            for (int i = (sheet.FirstRowNum + 1); i <= rowCount; i++)
            {
                ...
                var ms = BaseDT.DC_UTILITY_CAMP.Add(m);
                if (string.IsNullOrEmpty(jd) == false && string.IsNullOrEmpty(wd) == false)
                {
        ...
        public static string getCount()
        {
            ...
            Count = BaseDT.DC_UTILITY_CAMP.getNum(new DC_UTILITY_CAMP_SW {ORGNOS=SystemCls.getCurUserOrgNo()});
            return Count;
        }
        #endregion
    }
