   ...
   public  class WILD_LOCALANIMAL
    {
       ...
       public static Message Add(WILD_LOCALANIMAL_Model m)
        {
            ...
            if (arrBIOLOGICALTYPECODE.Length > 0)
            {
                ...
                {
                    if (!isExists(new WILD_LOCALANIMAL_SW { BYORGNO = m.BYORGNO, BIOLOGICALTYPECODE = arrBIOLOGICALTYPECODE[i] }))
                    {
       ...
       /// <returns>参见模型</returns>
       public static DataTable getDT(WILD_LOCALANIMAL_SW sw)
       {
           StringBuilder sb = new StringBuilder();
           sb.AppendFormat("SELECT WILD_LOCALANIMALID, BYORGNO, BIOLOGICALTYPECODE FROM WILD_LOCALANIMAL WHERE 1=1");
      ...
      /// <returns></returns>
       public static DataTable getDT(WILD_LOCALANIMAL_SW sw, out int total)
       {
           StringBuilder sb = new StringBuilder();
           sb.AppendFormat("  FROM WILD_LOCALANIMAL WHERE 1=1");
           if (string.IsNullOrEmpty(sw.BYORGNO) == false)
           {
               if (sw.BYORGNO.Substring(4, 11) == "00000000000")  //获取所有市的
                   sb.AppendFormat(" AND (SUBSTRING(BYORGNO,1,4) = '{0}')", ClsSql.EncodeSql(sw.BYORGNO.Substring(0, 4)));
           ...
           string sql ="SELECT WILD_LOCALANIMALID, BYORGNO, BIOLOGICALTYPECODE " + sb.ToString() + "ORDER BY BYORGNO";
       ...
       /// <param name="sw">参见模型</param>
       /// <returns>参见模型</returns>
       public static DataTable getDISDT(WILD_LOCALANIMAL_SW sw)
       {
           StringBuilder sb = new StringBuilder();
           sb.AppendFormat("SELECT DISTINCT(BIOLOGICALTYPECODE) FROM WILD_LOCALANIMAL WHERE 1=1");
           if (string.IsNullOrEmpty(sw.ORGNO) == false)
           {
               sb.AppendFormat(" AND BYORGNO = '{0}'", ClsSql.EncodeSql(sw.ORGNO));
           }
