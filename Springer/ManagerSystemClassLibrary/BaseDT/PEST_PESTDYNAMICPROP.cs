...
namespace ManagerSystemClassLibrary.BaseDT
{
    ...
    /// 有害生物__动态属性表
    /// </summary>
    public class PEST_PESTDYNAMICPROP
    {
        #region 保存
        /// <summary>
        /// 保存
        ...
        public static Message Save(PEST_PESTDYNAMICPROP_Model m)
        {
            ...
            if (arrDYNAMICPROPCODE.Length - 1 > 0)
            {
                ...
                sbDelete.AppendFormat("delete from PEST_PESTDYNAMICPROP where 1=1");
                ...
                sbInsert.AppendFormat("INSERT INTO PEST_PESTDYNAMICPROP(BIOLOGICALTYPECODE, DYNAMICPROPCODE, DYNAMICPROPCONTENT)");
                for (int i = 0; i < arrDYNAMICPROPCODE.Length - 1; i++)
                {
                    #region 更新
                    if (isExists(new PEST_PESTDYNAMICPROP_SW { BIOLOGICALTYPECODE = m.BIOLOGICALTYPECODE, DYNAMICPROPCODE = arrDYNAMICPROPCODE[i] }))
                    {
                        StringBuilder sbUpdate = new StringBuilder();
                        sbUpdate.AppendFormat("UPDATE PEST_PESTDYNAMICPROP SET ");
        ...
        /// <returns>参见模型</returns>
        public static DataTable getDT(PEST_PESTDYNAMICPROP_SW sw)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("SELECT * FROM PEST_PESTDYNAMICPROP WHERE 1=1");
