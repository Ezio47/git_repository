...
namespace ManagerSystemClassLibrary.BaseDT
{
    ...
    ///  防火系统部门与OA系统部门关联表
    /// </summary>
    public class T_SYS_Dept_OADept
    {
        ...
        public static Message DeptMap(T_SYS_Dept_OADept_Model m)
        {
            ...
            sbInsert.AppendFormat("Insert into T_SYS_Dept_OADept(SysORGNO,SysDeptID,OADeptID)");
            for (int i = 0; i < OADeptArray.Length; i++)
            {
                #region 更新
                if (DataBaseClass.JudgeRecordExists("select 1 from T_SYS_Dept_OADept where SysORGNO='" + m.SysORGNO + "' and SysDeptID='" + SysDeptArray[i] + "'"))
                {
                    StringBuilder sbUpdate = new StringBuilder();
                    if (OADeptArray[i] != "")
                    {
                        sbUpdate.AppendFormat("Update T_SYS_Dept_OADept SET ");
                    ...
                    {
                        sbUpdate.AppendFormat("Delete  from  T_SYS_Dept_OADept  where  SysORGNO='" + m.SysORGNO + "'  and SysDeptID='" + SysDeptArray[i] + "'");
                        sqllist.Add(sbUpdate.ToString());
                    }
                }
                #endregion
        ...
        public static string FindOADeptBySysDept(string sysORGNO, string sysDeptIdList)
        {
            ...
            {
                string sql = "select OADEPTID from T_SYS_Dept_OADept where SysORGNO='" + sysORGNO + "' and SysDeptID='" + sysDeptId[i] + "'";
