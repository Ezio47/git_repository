    ...
    public class FIRERECORD_REPORT10
    {
        ...
        public static Message Add(FIRERECORD_REPORT10_Model m)
        {
            ...
            for (int i = 0; i < arrREPORTVALUE.Length - 1; i++)
            {
                ...
                #region 更新
                if (isExists(new FIRERECORD_REPORT10_SW { BYORGNO = m.BYORGNO, REPORTYEAR = m.REPORTYEAR, REPORTCODE = arrREPORTCODE[i],}))
                {
        ...
        public static bool isExists(FIRERECORD_REPORT10_SW sw)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("select 1 from FIRERECORD_REPORT10 where 1=1");
            if (string.IsNullOrEmpty(sw.FIRERECORD_REPORT10ID) == false)
                sb.AppendFormat(" and FIRERECORD_REPORT10ID='{0}'", ClsSql.EncodeSql(sw.FIRERECORD_REPORT10ID));
