...
namespace ManagerSystemClassLibrary.BaseDT
{
    ...
    /// 数据中心_设施_监测站
    /// </summary>
    public class DC_UTILITY_MONITORINGSTATION
    {
        #region 添加
        /// <summary>
        /// 添加
        ...
        {
            if (DC_UTILITY_MONITORINGSTATION.isExistsPoint(new DC_UTILITY_MONITORINGSTATION_Model { JD = m.JD, WD = m.WD }) == false)
            {
            ...
            else { return new Message(false, "添加失败,已有相同的位置的监测站！", ""); }
        }
        ...
        /// <param name="m">参见模型</param>
        /// <returns>参见模型</returns>
        public static Message Del(DC_UTILITY_MONITORINGSTATION_Model m)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("delete DC_UTILITY_MONITORINGSTATION");
            sb.AppendFormat(" where DC_UTILITY_MONITORINGSTATION_ID= '{0}'", ClsSql.EncodeSql(m.DC_UTILITY_MONITORINGSTATION_ID));
            bool bln = DataBaseClass.ExeSql(sb.ToString());
            if (bln == true)
        ...
        public static bool isExists(DC_UTILITY_MONITORINGSTATION_SW sw)
        {

            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("select 1 from DC_UTILITY_MONITORINGSTATION where 1=1");
            if (string.IsNullOrEmpty(sw.DC_UTILITY_MONITORINGSTATION_ID) == false)
                sb.AppendFormat(" where DC_UTILITY_MONITORINGSTATION_ID= '{0}'", ClsSql.EncodeSql(sw.DC_UTILITY_MONITORINGSTATION_ID));
        ...
        /// <param name="sw"></param>
        /// <returns></returns>
        public static DataTable getDT(DC_UTILITY_MONITORINGSTATION_SW sw)
        {
            StringBuilder sb = new StringBuilder();

            sb.AppendFormat(" FROM      DC_UTILITY_MONITORINGSTATION");
            sb.AppendFormat(" WHERE   1=1");
            if (string.IsNullOrEmpty(sw.DC_UTILITY_MONITORINGSTATION_ID) == false)
                sb.AppendFormat(" AND DC_UTILITY_MONITORINGSTATION_ID = '{0}'", ClsSql.EncodeSql(sw.DC_UTILITY_MONITORINGSTATION_ID));
            ...
            string sql = "SELECT DC_UTILITY_MONITORINGSTATION_ID, NUMBER, NAME, ADDRESS, MODEL, BYORGNO,TRANSFERMODETYPE, MONICONTENT, BUILDDATE, USESTATE, MANAGERSTATE, MARK, JD, WD,WORTH,BUILDDATEBEGIN,BUILDDATEEND"
                + sb.ToString()
                + " order by DC_UTILITY_MONITORINGSTATION_ID desc";
        ...
        /// <returns></returns>
        public static DataTable getDT(DC_UTILITY_MONITORINGSTATION_SW sw, out int total)
        {
            StringBuilder sb = new StringBuilder();
            ...
            string sql = "SELECT DC_UTILITY_MONITORINGSTATION_ID, NUMBER, NAME, ADDRESS, MODEL, BYORGNO,TRANSFERMODETYPE, MONICONTENT, BUILDDATE, USESTATE, MANAGERSTATE, MARK, JD, WD,WORTH,BUILDDATEBEGIN,BUILDDATEEND"
                + sb.ToString()
                + " order by BYORGNO,DC_UTILITY_MONITORINGSTATION_ID desc";
        ...
        /// <param name="m"></param>
        /// <returns></returns>
        public static bool isExistsPoint(DC_UTILITY_MONITORINGSTATION_Model m)
        {
        ...
        #region 根据DataTable和OrgNo和监测站的各个类型判断记录个数
        /// <summary>
        ...
        public static string getCountMONITORINGSTATIONByOrgNo(DataTable dt, string orgNo, string DICTVALUE, string TYPE)
        {
            ...
            if (TYPE == "1")//统计使用现状类型
            {
                ...
                {
                    if (string.IsNullOrEmpty(DICTVALUE))
                        return dt.Compute("count(DC_UTILITY_MONITORINGSTATION_ID)", "BYORGNO='" + PublicCls.getZhenIncOrgNo(orgNo) + "'").ToString();
                    else
                        return dt.Compute("count(DC_UTILITY_MONITORINGSTATION_ID)", "BYORGNO='" + PublicCls.getZhenIncOrgNo(orgNo) + "' and USESTATE='" + DICTVALUE + "'").ToString();
                }
                else //机构编码可能不正确
                    return "";
            }
            if (TYPE == "2")//统计维护管理现状
            {
                ...
                else if (PublicCls.OrgIsZhen(orgNo))
                {
                    if (string.IsNullOrEmpty(DICTVALUE))
                        ...
                        return dt.Compute("count(DC_UTILITY_MONITORINGSTATION_ID)", "BYORGNO='" + PublicCls.getZhenIncOrgNo(orgNo) + "' and MANAGERSTATE='" + DICTVALUE + "'").ToString();
                }
                else //机构编码可能不正确
                    return "";
            }
            if (TYPE == "3")//无线电传输方式
            {
                ...
                else if (PublicCls.OrgIsZhen(orgNo))
                {
                    if (string.IsNullOrEmpty(DICTVALUE))
                        ...
                        return dt.Compute("count(DC_UTILITY_MONITORINGSTATION_ID)", "BYORGNO='" + PublicCls.getZhenIncOrgNo(orgNo) + "' and TRANSFERMODETYPE='" + DICTVALUE + "'").ToString();
                }
                else //机构编码可能不正确
                    return "";
            }
        ...
        #region 根据DataTable和OrgNo和监测站的各个市县类型判断记录个数
        /// <summary>
        ...
        /// <param name="sw"></param>
        /// <returns></returns>
        public static string getNum(DC_UTILITY_MONITORINGSTATION_SW sw)
        {
            string total = "";
            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("    from   DC_UTILITY_MONITORINGSTATION a ");
            sb.AppendFormat("where 1 = 1 ");
