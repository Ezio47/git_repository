...
namespace ManagerSystemClassLibrary.BaseDT
{
    ...
    /// 野生植物的动态属性
    /// </summary>
   public class WILD_BOTANYDYNAMICPROP
    {
        #region 保存
        /// <summary>
        /// 保存
       ...
       public static Message Save(WILD_BOTANYDYNAMICPROP_Model m)
        {
            ...
            if (arrDYNAMICPROPCODE.Length - 1 > 0)
            {
                ...
                sbDelete.AppendFormat("delete from WILD_BOTANYDYNAMICPROP where 1=1");
                ...
                sbInsert.AppendFormat("INSERT INTO WILD_BOTANYDYNAMICPROP(BIOLOGICALTYPECODE, DYNAMICPROPCODE, DYNAMICPROPCONTENT)");
                for (int i = 0; i < arrDYNAMICPROPCODE.Length - 1; i++)
                {
                    #region 更新
                    if (isExists(new WILD_BOTANYDYNAMICPROP_SW { BIOLOGICALTYPECODE = m.BIOLOGICALTYPECODE, DYNAMICPROPCODE = arrDYNAMICPROPCODE[i] }))
                    {
                        StringBuilder sbUpdate = new StringBuilder();
                        sbUpdate.AppendFormat("UPDATE WILD_BOTANYDYNAMICPROP SET ");
        ...
        /// <returns>参见模型</returns>
       public static DataTable getDT(WILD_BOTANYDYNAMICPROP_SW sw)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("SELECT * FROM WILD_BOTANYDYNAMICPROP WHERE 1=1");
       ...
       public static bool isExists(WILD_BOTANYDYNAMICPROP_SW sw)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("select 1 from WILD_BOTANYDYNAMICPROP where 1=1");
