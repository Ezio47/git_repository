...
namespace ManagerSystemClassLibrary.BaseDT.SDE
{
    ...
    public class TD_RESOURCE
    {
       ...
       /// <param name="m"></param>
       /// <returns></returns>
        public static Message Add(TD_RESOURCE_Model m)
       {
           StringBuilder sb = new StringBuilder();
           sb.AppendFormat("insert into ZIYUAN(OBJECTID,NAME,DISPLAY_X,DISPLAY_Y,Shape,category) values(");
       ...
       /// <param name="m"></param>
       /// <returns></returns>
        public static Message Mdy(TD_RESOURCE_Model m)
       {          
           if (TD_RESOURCE.isExists(new TD_RESOURCE_Model { OBJECTID = m.OBJECTID }) == false) //如果开始添加空的的经纬度之后再修改则插入空间库是添加
           {
               StringBuilder sb = new StringBuilder();
               sb.AppendFormat("insert into ZIYUAN(OBJECTID,NAME,DISPLAY_X,DISPLAY_Y,Shape,category) values(");
               sb.AppendFormat("{0},", ClsSql.saveNullField(m.OBJECTID));
       ...
       /// <param name="m"></param>
       /// <returns></returns>
        public static Message Del(TD_RESOURCE_Model m)
       {
           StringBuilder sb = new StringBuilder();
           sb.AppendFormat("delete ZIYUAN");
        ...
        /// <param name="m"></param>
        /// <returns></returns>
        public static bool isExists(TD_RESOURCE_Model m)
        {

            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("select 1 from ZIYUAN where 1=1");
