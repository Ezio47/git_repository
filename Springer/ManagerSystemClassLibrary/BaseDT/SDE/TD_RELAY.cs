...
namespace ManagerSystemClassLibrary.BaseDT.SDE
{
     ...
     public class TD_RELAY
    {
        ...
        /// <param name="m"></param>
        /// <returns></returns>
         public static Message Add(TD_RELAY_Model m)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("insert into ZHONGJIZHAN(OBJECTID,NAME,DISPLAY_X,DISPLAY_Y,Shape,category) values(");
        ...
        /// <param name="m"></param>
        /// <returns></returns>
         public static Message Mdy(TD_RELAY_Model m)
        {
            if (TD_RELAY.isExists(new TD_RELAY_Model { OBJECTID = m.OBJECTID }) == false)
            {
                StringBuilder sb = new StringBuilder();
                sb.AppendFormat("insert into ZHONGJIZHAN(OBJECTID,NAME,DISPLAY_X,DISPLAY_Y,Shape,category) values(");
                sb.AppendFormat("{0},", ClsSql.saveNullField(m.OBJECTID));
            ...
            {
                StringBuilder sb = new StringBuilder();
                sb.AppendFormat("UPDATE ZHONGJIZHAN");
                sb.AppendFormat(" set ");
        ...
        /// <param name="m"></param>
        /// <returns></returns>
         public static Message Del(TD_RELAY_Model m)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("delete ZHONGJIZHAN");
         ...
         /// <param name="m"></param>
         /// <returns></returns>
         public static bool isExists(TD_RELAY_Model m)
         {

             StringBuilder sb = new StringBuilder();
             sb.AppendFormat("select 1 from ZHONGJIZHAN where 1=1");
