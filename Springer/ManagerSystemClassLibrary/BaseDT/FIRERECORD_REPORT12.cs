...
namespace ManagerSystemClassLibrary.BaseDT
{
    ...
    public class FIRERECORD_REPORT12
    {
        ...
        public static Message Add(FIRERECORD_REPORT12_Model m)
        {
            ...
            for (int i = 0; i < arrREPORTVALUE.Length - 1; i++)
            {
                ...
                if (isExists(new FIRERECORD_REPORT12_SW { BYORGNO = m.BYORGNO, REPORTYEAR = m.REPORTYEAR, REPORTCODE = arrREPORTCODE[i], }))
                {
                    ...
                    sbUpdate.AppendFormat("UPDATE FIRERECORD_REPORT12 SET ");
        ...
        public static bool isExists(FIRERECORD_REPORT12_SW sw)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("select 1 from FIRERECORD_REPORT12 where 1=1");
            if (string.IsNullOrEmpty(sw.FIRERECORD_REPORT12ID) == false)
