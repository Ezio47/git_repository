...
namespace ManagerSystemClassLibrary.BaseDT
{
    ...
    /// 数据中心_设施_宣传碑牌
    /// </summary>
    public class DC_UTILITY_PROPAGANDASTELE
    {
        #region 添加
        /// <summary>
        /// 添加
        ...
        public static Message Add(DC_UTILITY_PROPAGANDASTELE_Model m)
        {
            if (DC_UTILITY_PROPAGANDASTELE.isExistsPoint(new DC_UTILITY_PROPAGANDASTELE_Model { JD = m.JD, WD = m.WD }) == false)
            {
                StringBuilder sb = new StringBuilder();
                //sb.AppendFormat("INSERT INTO  DC_UTILITY_PROPAGANDASTELE(PROPAGANDASTELETYPE, NUMBER, NAME, ADDRESS, BYORGNO, STRUCTURETYPE, BUILDDATE, USESTATE, MANAGERSTATE, MARK, JD, WD)");
                sb.AppendFormat("INSERT INTO  DC_UTILITY_PROPAGANDASTELE( PROPAGANDASTELETYPE, NUMBER, NAME, ADDRESS, BYORGNO, STRUCTURETYPE, BUILDDATE, USESTATE, MANAGERSTATE, MARK, JD, WD,WORTH,BUILDDATEBEGIN,BUILDDATEEND) output inserted.DC_UTILITY_PROPAGANDASTELE_ID");
                sb.AppendFormat(" VALUES(");
                sb.AppendFormat("{0}", ClsSql.saveNullField(m.PROPAGANDASTELETYPE));
            ...
            else { return new Message(false, "添加失败,已有相同的位置的宣传碑牌！", ""); }
        }
        ...
        /// <param name="m">参见模型</param>
        /// <returns>参见模型</returns>
        public static Message Mdy(DC_UTILITY_PROPAGANDASTELE_Model m)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("UPDATE DC_UTILITY_PROPAGANDASTELE");
            sb.AppendFormat(" set ");
            sb.AppendFormat("PROPAGANDASTELETYPE={0}", ClsSql.saveNullField(m.PROPAGANDASTELETYPE));
            ...
            sb.AppendFormat(" where DC_UTILITY_PROPAGANDASTELE_ID= '{0}'", ClsSql.EncodeSql(m.DC_UTILITY_PROPAGANDASTELE_ID));
            bool bln = DataBaseClass.ExeSql(sb.ToString());
            if (bln == true)
                return new Message(true, "修改成功！", m.DC_UTILITY_PROPAGANDASTELE_ID);
            else
        ...
        /// <param name="m">参见模型</param>
        /// <returns>参见模型</returns>
        public static Message MdyJWD(DC_UTILITY_PROPAGANDASTELE_Model m)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("UPDATE DC_UTILITY_PROPAGANDASTELE");
            sb.AppendFormat(" set ");
        ...
        /// <param name="m">参见模型</param>
        /// <returns>参见模型</returns>
        public static Message Del(DC_UTILITY_PROPAGANDASTELE_Model m)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("delete DC_UTILITY_PROPAGANDASTELE");
            sb.AppendFormat(" where DC_UTILITY_PROPAGANDASTELE_ID= '{0}'", ClsSql.EncodeSql(m.DC_UTILITY_PROPAGANDASTELE_ID));
            bool bln = DataBaseClass.ExeSql(sb.ToString());
            if (bln == true)
        ...
        public static bool isExists(DC_UTILITY_PROPAGANDASTELE_SW sw)
        {

            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("select 1 from DC_UTILITY_PROPAGANDASTELE where 1=1");
            if (string.IsNullOrEmpty(sw.DC_UTILITY_PROPAGANDASTELE_ID) == false)
                sb.AppendFormat(" where DC_UTILITY_PROPAGANDASTELE_ID= '{0}'", ClsSql.EncodeSql(sw.DC_UTILITY_PROPAGANDASTELE_ID));
        ...
        /// <param name="m"></param>
        /// <returns></returns>
        public static bool isExistsPoint(DC_UTILITY_PROPAGANDASTELE_Model m)
        {
        ...
        /// <param name="sw"></param>
        /// <returns></returns>
        public static DataTable getDT(DC_UTILITY_PROPAGANDASTELE_SW sw)
        {
            StringBuilder sb = new StringBuilder();

            sb.AppendFormat(" FROM      DC_UTILITY_PROPAGANDASTELE");
            sb.AppendFormat(" WHERE   1=1");
            if (string.IsNullOrEmpty(sw.DC_UTILITY_PROPAGANDASTELE_ID) == false)
                sb.AppendFormat(" AND DC_UTILITY_PROPAGANDASTELE_ID = '{0}'", ClsSql.EncodeSql(sw.DC_UTILITY_PROPAGANDASTELE_ID));
            ...
            if (string.IsNullOrEmpty(sw.PROPAGANDASTELETYPE) == false)
                sb.AppendFormat(" AND PROPAGANDASTELETYPE = '{0}'", ClsSql.EncodeSql(sw.PROPAGANDASTELETYPE));
            ...
            string sql = "SELECT DC_UTILITY_PROPAGANDASTELE_ID, PROPAGANDASTELETYPE, NUMBER, NAME, ADDRESS, BYORGNO, STRUCTURETYPE, BUILDDATE, USESTATE, MANAGERSTATE, MARK, JD, WD,WORTH,BUILDDATEBEGIN,BUILDDATEEND"
                + sb.ToString()
                + " order by DC_UTILITY_PROPAGANDASTELE_ID desc";
        ...
        /// <returns></returns>
        public static DataTable getDT(DC_UTILITY_PROPAGANDASTELE_SW sw, out int total)
        {
            StringBuilder sb = new StringBuilder();
            ...
            string sql = "SELECT DC_UTILITY_PROPAGANDASTELE_ID, PROPAGANDASTELETYPE, NUMBER, NAME, ADDRESS, BYORGNO, STRUCTURETYPE, BUILDDATE, USESTATE, MANAGERSTATE, MARK, JD, WD,WORTH,BUILDDATEBEGIN,BUILDDATEEND"
                + sb.ToString()
                + " order by BYORGNO,DC_UTILITY_PROPAGANDASTELE_ID desc";
        ...
        #region 根据DataTable和OrgNo和宣传碑牌的各个类型判断记录个数
        /// <summary>
        ...
        public static string getCountPROPAGANDASTELEByOrgNo(DataTable dt, string orgNo, string DICTVALUE, string TYPE)
        {
            ...
            if (TYPE == "1")//统计使用现状类型
            {
                ...
                {
                    if (string.IsNullOrEmpty(DICTVALUE))
                        return dt.Compute("count(DC_UTILITY_PROPAGANDASTELE_ID)", "BYORGNO='" + PublicCls.getZhenIncOrgNo(orgNo) + "'").ToString();
                    else
                        return dt.Compute("count(DC_UTILITY_PROPAGANDASTELE_ID)", "BYORGNO='" + PublicCls.getZhenIncOrgNo(orgNo) + "' and USESTATE='" + DICTVALUE + "'").ToString();
                }
                else //机构编码可能不正确
                    return "";
            }
            if (TYPE == "2")//统计维护管理现状
            {
                ...
                else if (PublicCls.OrgIsZhen(orgNo))
                {
                    if (string.IsNullOrEmpty(DICTVALUE))
                        ...
                        return dt.Compute("count(DC_UTILITY_PROPAGANDASTELE_ID)", "BYORGNO='" + PublicCls.getZhenIncOrgNo(orgNo) + "' and MANAGERSTATE='" + DICTVALUE + "'").ToString();
                }
                else //机构编码可能不正确
                    return "";
            }
            if (TYPE == "3")//宣传碑类型
            {
                ...
                else if (PublicCls.OrgIsZhen(orgNo))
                {
                    if (string.IsNullOrEmpty(DICTVALUE))
                        ...
                        return dt.Compute("count(DC_UTILITY_PROPAGANDASTELE_ID)", "BYORGNO='" + PublicCls.getZhenIncOrgNo(orgNo) + "' and PROPAGANDASTELETYPE='" + DICTVALUE + "'").ToString();
                }
                else //机构编码可能不正确
                    return "";
            }
            if (TYPE == "4")//结构类型
            {
                ...
                else if (PublicCls.OrgIsZhen(orgNo))
                {
                    if (string.IsNullOrEmpty(DICTVALUE))
                        ...
                        return dt.Compute("count(DC_UTILITY_PROPAGANDASTELE_ID)", "BYORGNO='" + PublicCls.getZhenIncOrgNo(orgNo) + "' and STRUCTURETYPE='" + DICTVALUE + "'").ToString();
                }
                else //机构编码可能不正确
                    return "";
            }
        ...
        #region 统计当前用户下宣传碑牌的数量
        /// <summary>
        /// 统计当前用户下宣传碑牌的数量
        /// </summary>
        /// <param name="sw"></param>
        /// <returns></returns>
        public static string getNum(DC_UTILITY_PROPAGANDASTELE_SW sw)
        {
            string total = "";
            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("    from   DC_UTILITY_PROPAGANDASTELE a ");
            sb.AppendFormat("where 1 = 1 ");
            if (sw.BYORGNO.Substring(4, 5) == "00000")//获取所有市的
                sb.AppendFormat(" AND (SUBSTRING(BYORGNO,1,4) = '{0}' or BYORGNO is null or BYORGNO='')", ClsSql.EncodeSql(sw.BYORGNO.Substring(0, 4)));
            else if (sw.BYORGNO.Substring(6, 3) == "000")//获取所有县的
