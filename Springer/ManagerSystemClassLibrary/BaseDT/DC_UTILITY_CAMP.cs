...
namespace ManagerSystemClassLibrary.BaseDT
{
    ...
    public class DC_UTILITY_CAMP
    {
        ...
        {
            if (DC_UTILITY_CAMP.isExistsPoint(new DC_UTILITY_CAMP_Model { JD = m.JD, WD = m.WD }) == false)
            {
        ...
        /// <param name="m">参见模型</param>
        /// <returns>参见模型</returns>
        public static Message Mdy(DC_UTILITY_CAMP_Model m)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("UPDATE DC_UTILITY_CAMP");
            sb.AppendFormat(" set ");
            ...
            sb.AppendFormat(" where DC_UTILITY_CAMP_ID= '{0}'", ClsSql.EncodeSql(m.DC_UTILITY_CAMP_ID));
            bool bln = DataBaseClass.ExeSql(sb.ToString());
            if (bln == true)
                return new Message(true, "修改成功！", m.DC_UTILITY_CAMP_ID);
            else
        ...
        /// <param name="m">参见模型</param>
        /// <returns>参见模型</returns>
        public static Message MdyJWD(DC_UTILITY_CAMP_Model m)
        {
            StringBuilder sb = new StringBuilder();
        ...
        /// <param name="m">参见模型</param>
        /// <returns>参见模型</returns>
        public static Message Del(DC_UTILITY_CAMP_Model m)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("delete DC_UTILITY_CAMP");
            sb.AppendFormat(" where DC_UTILITY_CAMP_ID= '{0}'", ClsSql.EncodeSql(m.DC_UTILITY_CAMP_ID));
            bool bln = DataBaseClass.ExeSql(sb.ToString());
            if (bln == true)
        ...
        public static bool isExists(DC_UTILITY_CAMP_SW sw)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("select 1 from DC_UTILITY_CAMP where 1=1");
            if (string.IsNullOrEmpty(sw.DC_UTILITY_CAMP_ID) == false)
                sb.AppendFormat(" where DC_UTILITY_CAMP_ID= '{0}'", ClsSql.EncodeSql(sw.DC_UTILITY_CAMP_ID));
        ...
        /// <param name="sw"></param>
        /// <returns></returns>
        public static DataTable getDT(DC_UTILITY_CAMP_SW sw)
        {
            StringBuilder sb = new StringBuilder();
        ...
        /// <param name="m"></param>
        /// <returns></returns>
        public static bool isExistsPoint(DC_UTILITY_CAMP_Model m)
        {
        ...
        #region 根据DataTable和OrgNo和营房的各个类型判断记录个数
        /// <summary>
        /// 根据DataTable和OrgNo和营房类型判断记录个数
        /// </summary>
        /// <param name="dt">DataTable</param>
        /// <param name="orgNo">单位编码</param>
        /// <param name="DICTVALUE">结构类型值</param> 
        /// <returns>记录个数</returns>
        public static string getCountCAMPByOrgNo(DataTable dt, string orgNo, string DICTVALUE)
        {
            ...
            if (PublicCls.OrgIsShi(orgNo))//市
            {
                if (string.IsNullOrEmpty(DICTVALUE))
                    return dt.Compute("count(DC_UTILITY_CAMP_ID)", "substring(ORGNOS,1,4)='" + PublicCls.getShiIncOrgNo(orgNo) + "'").ToString();
                else
                    return dt.Compute("count(DC_UTILITY_CAMP_ID)", "substring(ORGNOS,1,4)='" + PublicCls.getShiIncOrgNo(orgNo) + "' and STRUCTURETYPE='" + DICTVALUE + "'").ToString();
            }
            else if (PublicCls.OrgIsXian(orgNo))//县
            {
                if (string.IsNullOrEmpty(DICTVALUE))
                    return dt.Compute("count(DC_UTILITY_CAMP_ID)", "substring(ORGNOS,1,6)='" + PublicCls.getXianIncOrgNo(orgNo) + "'").ToString();
                else
                    return dt.Compute("count(DC_UTILITY_CAMP_ID)", "substring(ORGNOS,1,6)='" + PublicCls.getXianIncOrgNo(orgNo) + "' and STRUCTURETYPE='" + DICTVALUE + "'").ToString();
            }
            else if (PublicCls.OrgIsZhen(orgNo))
            {
                if (string.IsNullOrEmpty(DICTVALUE))
                    return dt.Compute("count(DC_UTILITY_CAMP_ID)", "substring(ORGNOS,1,9)='" + PublicCls.getZhenIncOrgNo(orgNo) + "'").ToString();
                else
                    return dt.Compute("count(DC_UTILITY_CAMP_ID)", "substring(ORGNOS,1,9)='" + PublicCls.getZhenIncOrgNo(orgNo) + "' and STRUCTURETYPE='" + DICTVALUE + "'").ToString();
            }
            else //机构编码可能不正确
                return "";
        }
        ...
        #region 根据DataTable和OrgNo和营房的各个类型判断市县记录个数
        /// <summary>
        /// 根据DataTable和OrgNo和营房类型判断记录个数
        /// </summary>
        /// <param name="dt">DataTable</param>
        ...
        public static string getCountXSByOrgNo(DataTable dt, string orgNo, string DICTVALUE)
        {
            ...
            {
                if (string.IsNullOrEmpty(DICTVALUE))
                    return dt.Compute("count(DC_UTILITY_CAMP_ID)", "ORGNOS='" + orgNo + "'").ToString();
                else
                    return dt.Compute("count(DC_UTILITY_CAMP_ID)", "ORGNOS='" + orgNo + "' and STRUCTURETYPE='" + DICTVALUE + "'").ToString();
            }
            else //机构编码可能不正确
                return "";
        }
        ...
        /// <param name="sw"></param>
        /// <returns></returns>
        public static string getNum(DC_UTILITY_CAMP_SW sw)
        {
            string total = "";
            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("    from    DC_UTILITY_CAMP a ");
            sb.AppendFormat("where 1 = 1 ");
            if (sw.ORGNOS.Substring(4, 11) == "00000000000")//获取所有市的
                sb.AppendFormat(" AND (SUBSTRING(ORGNOS,1,4) = '{0}' or ORGNOS is null or ORGNOS='')", ClsSql.EncodeSql(sw.ORGNOS.Substring(0, 4)));
            else if (sw.ORGNOS.Substring(6, 9) == "000000000")//获取所有县的
