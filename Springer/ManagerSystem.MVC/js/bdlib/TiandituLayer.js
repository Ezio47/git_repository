    ...
    function (declare) {
        return declare(esri.layers.TiledMapServiceLayer, {
    		...
    		getTileUrl: function (level, row, col) {
    			...
    			return "http://t1.tianditu.cn/DataServer?T=" + levelMap + "&" + 
    					"X=" + col + "&" + "Y=" + row + "&" + "L=" + (level * 1 + 1);
    		}
			**/
			
 			getTileUrl: function(level, row, col) {
				return "http://t3.tianditu.cn/vec_c/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=vec&STYLE=default&TILEMATRIXSET=c&TILEMATRIX="+level+"&TILEROW="+row+"&TILECOL="+col+"&FORMAT=tiles";
			}
        });
    });